...
const upload = multer({
  storage,
  fileFilter: function(req, file: Express.Multer.File, cb) {
    const allowedTypes = ["image/jpeg", "image/png", "image/webp"];
    const allowedExtensions = [".jpeg", ".jpg", ".png", "webp"]; // might not need this line
  ...
  app.post(
    ...
    async (req: Request, res: Response<ApiResponse<string>>) => {
      ...
      try {
        ...
        /*
           TODO: Handle multer errors
           => (err instanceof multer.MulterError)
           => If used as middleware Multer will delegate error to Express, so this might not be necessary
        */
        if (isDbError(error)) {
